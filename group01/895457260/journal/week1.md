...
回到问题：如何进行计算？首先要有一个东西，能从外部接受2个数据，然后把它们加减乘除与或非，再传出一个结果，而且还能随意控制到底做加法还是减法，还是别的。这个东西叫“ALU”，就是Arithmetic Logic Unit，算术逻辑单元。它实际上除了能接受2个数据进行计算以外，还能接受一堆别的数据（信号），用来控制做哪种计算，这些额外的输入就叫做“控制信号”。
输入：进行计算的数据2个 + 控制信号若干
输出：计算结果
有了ALU，就可以做各种各样的计算。
但是问题又来了：数据从哪来？
...
解决了数据的来源，接下来是ALU控制信号从哪来的问题。回答仍然是：内存。
