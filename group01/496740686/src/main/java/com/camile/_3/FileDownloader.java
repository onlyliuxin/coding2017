...
import com.camile._3.api.DownloadListener;
...
public class FileDownloader {
	...
	public void execute() {
		...
		Connection conn = (Connection) cm.open(this.url);
		int length = ((Connection) conn).getContentLength();
		conn.close();
		...
		try (FileOutputStream fos = new FileOutputStream(new File("download.jpg"))) {
			for (DownloadThread thread : threads) {
