...
import com.basic.week3.download.api.DownloadListener;
...
public class FileDownloader {
	...
	public void execute(){
		...
		try {
			...
			int length=conn.getContentLength();
			//每个线程所占的资源
			int off=(int)(length/3);
			int start=0;
			for(int i=0;i<THREAD_NUM-1;i++){
				if(i==2){
					new DownloadThread(conn,start+off,length-2*off).start();
					break;
				}
				new DownloadThread(conn,start,off).start();
				start+=off;
			}
			while(true){
				if(DownloadThread.count==0){
					break;
				}
			}
		} catch (ConnectionException e) {
