...
public class StackFrame {
	...
	public ExecutionResult execute(){
		...
		// index记录cmds执行到了哪一条命令，如果遇到暂停并执行下一个函数帧的情况，等下次再调用此方法的时候，从上次暂停的下一条命令开始执行
		while(index<cmds.length){
			// result 用来记录command和栈帧的执行结果
			ExecutionResult result = new ExecutionResult();
			//缺省值是执行下一条命令  ----------有些命令执行完之后没有对result的nextAction进行设置，如NoOperandCommand上的一些命令
			result.setNextAction(ExecutionResult.RUN_NEXT_CMD);
			System.out.println(cmds[index].toString(m.getClzFile().getConstantPool()));
