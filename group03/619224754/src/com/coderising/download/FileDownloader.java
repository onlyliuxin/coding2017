...
public class FileDownloader {
	...
	private static final String BASE_PATH = "D:/learn/";
	...
	public void execute(){
		...
		try {
			conn = this.cm.open(this.url);
			int length = conn.getContentLength();
			String filePath = BASE_PATH + "download." + this.getFileTpye(this.url);
			...
			for (int i = 0; i < THREAD_NUM; i++) {
				...
				new Thread(new DownloadThread(barrier, conn, startPos, endPos, filePath)).start();
			}
	...
	private String getFileTpye(String url){
