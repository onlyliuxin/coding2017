...
public class FileDownloader {
	...
	public void execute() {
		...
		try {
			...
			new DownloadThread(conn, 0, length - 1, file, threadsCnt).start();
			int clength = length / threadNum;
			int off = 0;
			for (int i = 0; i < threadNum; i++) {
				if (i != threadNum - 1) {
					new DownloadThread(cm.open(url), off, off + clength, file,threadsCnt).start();
				} else {
					new DownloadThread(cm.open(url), off, length - 1, file,threadsCnt).start();
				}
				off = off + clength + 1;
			}
			threadsCnt.await();
