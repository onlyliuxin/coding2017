...
public class FileDownloader {
	...
	private static final String DOWNLOAD_PATH = "E:/downloadTest/";


	public FileDownloader(String _url) {
		this.url = _url;
		File baseFile = new File(DOWNLOAD_PATH);
		if(!baseFile.exists()){
			baseFile.mkdirs();
		}
	...
	public void execute(){
		...
		try {
			...
			conn = conMan.open(this.url);

			int length = conn.getContentLength();
			
			String filePath = DOWNLOAD_PATH + "多线程下载."+getFileType(this.url);//获取文件类型
