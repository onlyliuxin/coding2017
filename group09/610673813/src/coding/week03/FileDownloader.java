...
import coding.week03.download.api.DownloadListener;
...
public class FileDownloader {
	...
	public void execute() {
		...
		try {
			...
			int clength = length / threadNumber;
			int off = 0;
			for (int i = 0; i < threadNumber; i++) {
				if (i != threadNumber - 1) {
					new DownloadThread(threadSignal,cm.open(url), off, off + clength, file).start();
				} else {
					new DownloadThread(threadSignal,cm.open(url), off, length - 1, file).start();
				}
				off = off + clength + 1;
			}
			threadSignal.await();
